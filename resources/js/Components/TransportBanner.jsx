...
export const TransportBanner = ({ content }) => {
    ...
    return (
        <section
            className="relative bg-no-repeat bg-fill 2xl:bg-cover min-w-screen h-[60dvh] md:h-screen max-h-[672px]"
            style={{
            ...
            <div className="container max-w-large h-full">
                <div className="flex flex-col justify-center h-full pb-10">
                    {partesTitulo.map((parte, i) => (
                        <LetterReveal
                            key={i}
                            className={`font-secondary ${i === 0 ? "text-primary" : "text-secondary"} text-center text-4xl sm:text-5xl md:text-6xl 2xl:text-7xl font-black leading-[1.2] -mt-2 uppercase`}
                            text={parte.trim()}
                            element="h1"
                        />
                    ))}
