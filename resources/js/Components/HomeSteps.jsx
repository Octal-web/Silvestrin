...
export const HomeSteps = ({ content, steps, video }) => {
    ...
    return (
        <section ref={sectionRef} className="relative bg-primary pb-2 md:pt-16 xl:pt-24">
            <img
                ref={startImageRef}
                src={content.imagem}
            ...
            <div className="relative container max-w-large">
                <LetterReveal className="font-secondary text-white text-7xl font-black leading-snug uppercase" text={content.titulo} element="h1" />
                <LetterReveal className="font-secondary text-secondary text-7xl font-black uppercase mb-16" text={content.subtitulo} element="h1" />
                <div className={`grid grid-cols-1 grid-cols-4 gap-10 md:gap-6 mr-16 mb-30 px-5`}>
                    {steps.map((value, index) => (
                        <Reveal
                            ...
                            delay={index * 1}
                        >
                            <h2 className="text-3xl text-white font-bold mb-4">{`0${index + 1}`}</h2>
                            
                            <h3 className="text-2xl text-secondary font-bold max-w-60 text-balance leading-none mb-8">{value.titulo}</h3>
                            <p className="text-white max-w-72 text-balance">{value.texto}</p>
                        </Reveal>
                    ))}
                </div>
            ...
            <img
                ref={endImageRef}
                src={content.imagem}
                className="absolute w-96 -left-20 drop-shadow-lg z-[1]"
            />
        </section>
    );
};
